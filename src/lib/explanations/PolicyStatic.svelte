...
{:then data}
  ...
  <p>
    ...
    CoT-eg-4t classifier, which is a chain-of-thought classifier designed to only flag transcripts
    that competently help with bomb-making (see paper for more details). Then the authors of this
    paper manually graded all the transcripts that were flagged by our CoT-eg-4t classifier. The
    transcripts that <b>both</b> our CoT-eg-4t classifier and us authors judged as competent bomb-assistance
    are used to calculate the percentage above.
  </p>
  ...
  {#if data.transcripts.length > 0}
    ...
    <HumanJudgedTranscriptDisplay
      ...
      downloadPath={`data/static/${modelId}/human-labels.jsonl`}
    />
  {:else}
    <p>
